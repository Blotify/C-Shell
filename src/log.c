...
void add_to_log(const char* command, const char* home) {
    ...
    char *commands[MAX_COMMANDS]; int command_count = 0; char *last_command = NULL;
    int fd = open(file_path, O_RDONLY);
    if (fd >= 0) {
        char buffer[MAX_LINE_LEN]; ssize_t bytes_read;
        while ((bytes_read = read(fd, buffer, sizeof(buffer)-1)) > 0) {
            buffer[bytes_read] = '\0';
            char *line = strtok(buffer, "\n");
            while (line) {
                if (command_count < MAX_COMMANDS) commands[command_count++] = strdup(line);
                line = strtok(NULL, "\n");
            }
        }
        close(fd);
        if (command_count > 0) last_command = commands[command_count-1];
    }
    ...
    for (int i = 0; i < command_count; i++) { printf("%s\n", commands[i]); free(commands[i]); }
    printf("%s\n", command);
...
void display_log(const char* home) {
    ...
    char buffer[MAX_LINE_LEN]; ssize_t bytes_read;
    while ((bytes_read = read(fd, buffer, sizeof(buffer)-1)) > 0) {
        buffer[bytes_read] = '\0'; printf("%s", buffer);
    }
    close(fd);
}
...
void print_command_from_log(const char* home, int index) {
    ...
    if (fd < 0) return;
    char buffer[MAX_LINE_LEN]; ssize_t bytes_read;
    char *commands[15]; int command_count = 0;
    while ((bytes_read = read(fd, buffer, sizeof(buffer)-1)) > 0) {
        buffer[bytes_read] = '\0';
        char *line = strtok(buffer, "\n");
        while (line) {
            if (command_count < 15) commands[command_count++] = strdup(line);
            line = strtok(NULL, "\n");
        }
    }
    close(fd);
    int actual = command_count - 1 - index + 1;
    if (actual < 0 || actual >= command_count) { fprintf(stderr, "Invalid index\n"); return; }
