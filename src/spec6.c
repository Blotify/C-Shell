        ...
        if (!flag) {
            ...
            if (idx != -1) {
                if (WIFEXITED(status) || WIFSIGNALED(status)) {
                    ...
                    processes[idx].end_time = time(NULL);
                } else if (WIFSTOPPED(status)) {
                    processes[idx].state = 2;
                }
            }
...
void get_more_process_info(ProcessInfo **procs, int *count) {
    int c = 0;
    for (int i = 0; i < MAX_PROCS; i++) {
        if (processes[i].pid != 0) {
            c++;
    ...
    *procs = malloc(sizeof(ProcessInfo) * c);
    int j = 0;
    for (int i = 0; i < MAX_PROCS; i++) {
        if (processes[i].pid != 0) {
            (*procs)[j].pid = processes[i].pid;
            (*procs)[j].state = processes[i].state;
            (*procs)[j].end_time = processes[i].end_time;
            (*procs)[j].name = strdup(processes[i].name);
            j++;
        }
    }
    *count = c;
...
void cleanup_finished_processes() {
    for (int i = 0; i < MAX_PROCS; i++) {
        if (processes[i].pid != 0 && processes[i].state == 1) {
